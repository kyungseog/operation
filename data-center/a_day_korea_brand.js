...
async function updateDayKoreaBrand() {
  const salesSql = `
    SELECT b.brand_id 
      , c.brand_name
      , c.account_type 
  ...
  const searchDays = [];
  for (let i = 1; i <= 20; i++) {
    searchDays.push(DateTime.now().minus({ days: i }).toFormat("yyyy-LL-dd"));
  }

  for (let searchDay of searchDays) {
    let uploadData = [];
    const salesData = await util.sqlData(salesSql, [searchDay]);
    const directMarketingData = await util.sqlData(directMarketingSql, [searchDay]);
    const indirectMarketingData = await util.sqlData(indirectMarketingSql, [searchDay]);
    const logisticData = await util.sqlData(logisticSql, [searchDay]);
    ...
    for (let i = 0; i < uploadData.length; i++) {
      const updateSql = `
      INSERT INTO management.day_korea_brands
      SET ?
      ...
      await util.sqlData(updateSql, uploadData[i]);
      util.delayTime(100);
    }
    console.log(searchDay, "update complete");
    util.delayTime(500);
  }
}
